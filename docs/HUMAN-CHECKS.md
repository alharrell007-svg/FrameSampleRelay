...
Completed on rc.2: installation through the generated button; fresh-chat filesystem discovery followed by video analysis; normal LM Studio close/reopen and a new successful call; candidate uninstall/reinstall and a new successful call. The original integration remained installed. These observations are inherited by rc.3 because its setup/server logic is unchanged, not claimed as new rc.3 desktop tests.
...
5. After publication is authorized and a repository exists, configure and verify GitHub private vulnerability reporting before advertising it as available. If unavailable, ask the owner for an approved alternative; no email substitute is authorized.
